  ...
  for ( UInt_t i=0; i<N; i++ )
    {
      ...
      else Error("CalculateValues", "Division by zero(i): sigma0");
      ...
      for ( UInt_t j=0; j<nds; j++ )
	{
	  ...
	  else Error("CalculateValues", "Division by zero(j): sigma0");
      ...
      for ( Int_t n = 2; n<50; n++ )
	{
	  ...
	  else Error("CalculateValues", "Division by zero: sigmaN");
