...
TH1D *Deconvolution::CleanUps(TH1D *h)
{
   ...
   UInt_t N = n2 - n1 + 1; // std::cout << N << std::endl;
...
{
   std::cout << " Deconvolution starts ... " << std::endl;
   std::cout << " " << std::endl;
   // std::cout << "[" << flush;
   ...
   for (Int_t i = 0; i < n; i++)
      xx[i] = h4->GetXaxis()->GetBinCenter(i + 1);
   delete h4;
   ...
   for (Int_t i = 0; i < ntoys; i++) {
      ...
      for (Int_t j = 0; j < n; j++)
         yy[j] += h4->GetBinContent(j + 1);
      for (Int_t j = 0; j < n; j++)
         eyy[j] += h4->GetBinContent(j + 1) * h4->GetBinContent(j + 1);

      delete h4;

      std::cout << "\r Progress: " << Form("%.1f", i * 1.0 / ntoys * 100.0) << " %";
      std::cout.flush();
   }
   ...
   for (Int_t j = 0; j < n; j++)
      yy[j] = yy[j] / (1.0 * ntoys);
   for (Int_t j = 0; j < n; j++)
      eyy[j] = sqrt(eyy[j] / (1.0 * ntoys) - yy[j] * yy[j]);
   ...
   std::cout << " " << std::endl;
   std::cout << " " << std::endl;
   std::cout << " ... and now it ends !" << std::endl;
