...
PMTModel::PMTModel(Int_t _nbins, Double_t _xmin, Double_t _xmax, PMType::Model _modtype)
{
   ...
   if (_modtype == PMType::SIMPLEGAUSS)
      nparams = 8;
   if (_modtype == PMType::TRUNCGAUSS)
      nparams = 8;
   if (_modtype == PMType::ANATRUNCG)
      nparams = 8;
   if (_modtype == PMType::EXPTRUNCG)
      nparams = 8;
}
...
Double_t PMTModel::GetValue(Double_t xx)
{
   ...
   if (modtype == PMType::SIMPLEGAUSS)
      result = F1(xx);
   if (modtype == PMType::TRUNCGAUSS)
      result = F2(xx);
   if (modtype == PMType::ANATRUNCG)
      result = F3(xx);
   if (modtype == PMType::EXPTRUNCG)
      result = F4(xx);
